...
void handleAdcSamplingFiltered();
...
void handleAdcSamplingFiltered() {
  ...
  filterSum -= static_cast<uint32_t>(filterBuffer[filterIndex]);
  filterSum += static_cast<uint32_t>(adcValue);
  filterBuffer[filterIndex] = adcValue;

  filterIndex++;
  if (filterIndex >= FILTER_WINDOW_SIZE) {
    filterIndex = 0;
    filterFilled = true;
  }
  ...
  uint32_t divisor;
  if (filterFilled) {
    divisor = static_cast<uint32_t>(FILTER_WINDOW_SIZE);
  } else {
    divisor = (filterIndex == 0) ? 1U : static_cast<uint32_t>(filterIndex);
  }

  uint32_t filteredValue = filterSum / divisor;

  bool decidedBit = (filteredValue > static_cast<uint32_t>(ADC_THRESHOLD));
