...
int medianBuffer[FILTER_WINDOW_SIZE];
int medianIndex = 0;
...
void handleAdcSamplingFiltered() {
  ...
  medianBuffer[medianIndex] = adcValue;
  medianIndex++;
  if (medianIndex >= FILTER_WINDOW_SIZE) {
    medianIndex = 0;
    medianFilled = true;
  }
  ...
  int medianValue = temp[count / 2];

  bool decidedBit = (medianValue > ADC_THRESHOLD);
